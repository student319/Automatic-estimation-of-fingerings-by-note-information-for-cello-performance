...
for training_size in  [17]: #no unlabled training
    for tekl in range(t_size): #!!!!!! #which song to leave out for testing
        ...
        if len(labelled_key_list) > 0:
            ...
            # ベストエポックの取得
            if cal_earlystopping.stopped_epoch > 0:
                best_epoch = cal_earlystopping.stopped_epoch - cal_earlystopping.patience + 1
                best_epochs.append(best_epoch)
            else:
                best_epoch = max(history.epoch) + 1  # EarlyStoppingが発動しなかった場合
                best_epochs.append(best_epoch)
        ...
        output_path = f"review/fine_true({str(tekl)}).png"
        ...
        # Save the plot as a PNG file for easy viewing in Visual Studio
        output_path = f"review/fine_pred({str(tekl)}).png"
        plt.savefig(output_path)
        ...
        output_path = f"review/fine_txt({str(tekl)}).txt"
        ...
        plt.legend()
        plt.grid(True)
        output_path = f"review/fine_dist({str(tekl)}).png"
        plt.savefig(output_path)
        ...
        plt.legend()
        output_path = f"review/fine_ct({str(tekl)}).png"
        plt.savefig(output_path)
...
plt.tight_layout()
plt.savefig("review/fine_accuracy_boxplot.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/fine_midi_accuracy_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/fine_movement_distance_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/fine_movement_count_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/fine_movement_distance_boxplot.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/fine_movement_count_boxplot.png")
...
# 結果をテキストファイルに保存
output_path = "review/fine_statistics_summary.txt"
...
# テキストファイルに保存
output_path = "review/best_epoch.txt"
with open(output_path, "w") as f:
    f.write("=== ファインチューニングのベストエポック数 ===\n")
    f.write(f"ベストエポック数: {best_epochs} epochs\n")