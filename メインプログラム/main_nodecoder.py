...
if (pretrain_needed == 1):
    ...
    for training_size in  [13]: #no unlabled training
        for tekl in range(size): #!!!!!! #which song to leave out for testing
            ...
            M2l = keras.Model(emb_inl, classified_l, name='labelled')
            ...
            validation_accuracy = history.history['val_accuracy'][-11]
    ...
    #%% clear and delete
            ...
            del(embedder, blstm, classifier, M2l )
    ...
    if savemodel:
        print("\nSaving models...")
        best_embedder.save(model_name + '_n_emb.h5')
        #best_encoder.save(model_name + '_enc.h5')
        best_blstm.save(model_name + '_n_bls.h5')
        #best_decoder.save(model_name + '_dec.h5')
        best_classifier.save(model_name + '_n_cla.h5')
        #np.save(model_name+'.npy',history.history)
        print("Saved\n")
        #exit()
    
    output_file = 'review/training_time_no.txt'
...
#    best_encoder = keras.models.load_model("bestmodel_n_enc.h5")
best_blstm = keras.models.load_model("bestmodel_n_bls.h5")
best_embedder = keras.models.load_model("bestmodel_n_emb.h5")
...
for training_size in  [17]: #no unlabled training
    for tekl in range(t_size): #!!!!!! #which song to leave out for testing
        ...
        output_path = f"review/no_true({str(tekl)}).png"
        ...
        # Save the plot as a PNG file for easy viewing in Visual Studio
        output_path = f"review/no_pred({str(tekl)}).png"
        plt.savefig(output_path)
        ...
        output_path = f"review/no_txt({str(tekl)}).txt"
        ...
        plt.legend()
        plt.grid(True)
        output_path = f"review/no_dist({str(tekl)}).png"
        plt.savefig(output_path)
        ...
        plt.legend()
        output_path = f"review/no_ct({str(tekl)}).png"
        plt.savefig(output_path)
...
plt.tight_layout()
plt.savefig("review/no_accuracy_boxplot.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/no_midi_accuracy_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/no_movement_distance_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/no_movement_count_bar.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/no_movement_distance_boxplot.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("review/no_movement_count_boxplot.png")
...
# 結果をテキストファイルに保存
output_path = "review/no_statistics_summary.txt"
...
output_file = 'review/training_trans_time_no.txt'
